			    ...
			    GtkOrientation  orientation)
{
	...
	switch (view->pending_scroll) {
    	        case SCROLL_TO_KEEP_POSITION:
    	        case SCROLL_TO_FIND_LOCATION:
			new_value = CLAMP ((int)(upper * factor + 0.5), 0, (int)(upper - page_size + 0.5));
			...
			break;
    	        case SCROLL_TO_CENTER:
			new_value = CLAMP ((int)(upper * factor - zoom_center + 0.5), 0, (int)(upper - page_size + 0.5));
			if (orientation == GTK_ORIENTATION_HORIZONTAL)
...
view_update_range_and_current_page (EvView *view)
{
	...
	if (view->continuous) {
		...
		current_area.x = (int)(gtk_adjustment_get_value (view->hadjustment) + 0.5);
		current_area.width = (int)(gtk_adjustment_get_page_size (view->hadjustment) + 0.5);
		current_area.y = (int)(gtk_adjustment_get_value (view->vadjustment) + 0.5);
		current_area.height = (int)(gtk_adjustment_get_page_size (view->vadjustment) + 0.5);
			  ...
			  GtkScrollType  scroll)
{
	...
	rect.x = (int)(doc_rect.x1 + 0.5);
	rect.y = (int)(doc_rect.y1 + 0.5);
	rect.width = (int)(doc_rect.x2 - doc_rect.x1 + 0.5);
	rect.height = MAX (1, (int)(doc_rect.y2 - doc_rect.y1 + 0.5));
...
ensure_rectangle_is_visible (EvView *view, GdkRectangle *rect)
{
	...
	if (rect->y < adj_value) {
		value = MAX ((int)(gtk_adjustment_get_lower (adjustment) + 0.5), rect->y - MARGIN);
		gtk_adjustment_set_value (view->vadjustment, value);
	} else if (rect->y + rect->height > adj_value + allocation.height) {
		value = MIN ((int)(gtk_adjustment_get_upper (adjustment) + 0.5), rect->y + rect->height -
	...
	if (rect->x < adj_value) {
		value = MAX ((int)(gtk_adjustment_get_lower (adjustment) + 0.5), rect->x - MARGIN);
		gtk_adjustment_set_value (view->hadjustment, value);
	} else if (rect->x + rect->height > adj_value + allocation.width) {
		value = MIN ((int)(gtk_adjustment_get_upper (adjustment) + 0.5), rect->x + rect->width -
					    ...
					    GdkPoint *view_point)
{
	...
	view_x = CLAMP ((int)((x * view->scale) + 0.5), 0, page_area.width);
	view_y = CLAMP ((int)((y * view->scale) + 0.5), 0, page_area.height);
	view_point->x = (int)(view_x + page_area.x + 0.5);
	view_point->y = (int)(view_y + page_area.y + 0.5);
}
					  ...
					  GdkRectangle *view_rect)
{
	...
	view_rect->x = (int)(x * view->scale + page_area.x + 0.5);
	view_rect->y = (int)(y * view->scale + page_area.y + 0.5);
	view_rect->width = (int)(w * view->scale + 0.5);
	view_rect->height = (int)(h * view->scale + 0.5);
}
		       ...
		       gint    *y_offset)
{
	...
	for (i = view->start_page; i >= 0 && i <= view->end_page; i++) {
		...
		if ((x >= page_area.x + border.left) &&
			...
			*page = i;
			*x_offset = (int)(x - (page_area.x + border.left) + 0.5);
			*y_offset = (int)(y - (page_area.y + border.top) + 0.5);
			return;
		}
	}
		  ...
		  gdouble  y)
{
	...
	if (region)
		return cairo_region_contains_point (region,
		                                    (int)((x_offset / view->scale) + 0.5),
		                                    (int)((y_offset / view->scale) + 0.5));
	else
		return FALSE;
}
			   ...
			   GdkEventButton *event)
{
	...
	view->selection_info.start.x = (int)(event->x + view->scroll_x + 0.5);
	view->selection_info.start.y = (int)(event->y + view->scroll_y + 0.5);
			...
			if (first_line_offset == -1) {
				if (doc_x <= rect->x1) {
					...
					if (last_line_offset != -1) {
						...
						dx1 = (int)(doc_x - last->x2 + 0.5);
						dx2 = (int)(rect->x1 - doc_x + 0.5);
	...
	switch (event->button) {
			...
			if (EV_IS_SELECTION (view->document) && view->selection_info.selections) {
				if (event->type == GDK_3BUTTON_PRESS) {
					...
					end_point.x = (int)(event->x + view->scroll_x + 0.5);
					end_point.y = (int)(event->y + view->scroll_y + 0.5);
				...
				view->image_dnd_info.start.x = (int)(event->x + view->scroll_x + 0.5);
				view->image_dnd_info.start.y = (int)(event->y + view->scroll_y + 0.5);
			} else {
				ev_view_remove_all (view);
				_ev_view_set_focused_element (view, NULL, -1);
		...
		case 2:
			...
			view->drag_info.start.x = (int)(event->x_root + 0.5);
			view->drag_info.start.y = (int)(event->y_root + 0.5);
			...
			return TRUE;
		case 3:
			view->scroll_info.start_y = (int)(event->y + 0.5);
			ev_view_set_focused_element_at_location (view, event->x, event->y);
...
ev_view_scroll_drag_release (EvView *view)
{
	...
	view->drag_info.momentum.x = (int)((view->drag_info.momentum.x / 1.2) + 0.5);
	view->drag_info.momentum.y = (int)((view->drag_info.momentum.y / 1.2) + 0.5); /* Alter these constants to change "friction" */
	...
	case 2:
		if (!view->drag_info.in_drag) {
			...
			for (i = 0; i < DRAG_HISTORY; i++) {
				view->drag_info.buffer[i].x = (int)(event->x + 0.5);
				view->drag_info.buffer[i].y = (int)(event->y + 0.5);
			}
			view->drag_info.momentum.x = 0;
			view->drag_info.momentum.y = 0;
		}
		...
		if (view->drag_info.in_drag) {
			...
			view->drag_info.buffer[0].x = (int)(event->x + 0.5);
			view->drag_info.buffer[0].y = (int)(event->y + 0.5);

			dx = (int)(event->x_root - view->drag_info.start.x + 0.5);
			dy = (int)(event->y_root - view->drag_info.start.y + 0.5);
			      ...
			      GdkEventButton *event)
{
	...
	if (view->adding_annot && view->pressed_button == 1) {
		view->adding_annot = FALSE;
		ev_view_handle_cursor_over_xy (view, (int)(event->x + 0.5), (int)(event->y + 0.5));
		view->pressed_button = -1;

		ev_view_create_annotation (view,
					   view->adding_annot_type,
					   (int)(event->x + view->scroll_x + 0.5),
					   (int)(event->y + view->scroll_y + 0.5));
	      ...
	      gint             target_height)
{
	...
	if (width != target_width || height != target_height) {
		...
		offset_x = (int)((offset_x / scale_x) + 0.5);
		offset_y = (int)((offset_y / scale_y) + 0.5);
	}
